...
# OIDC / OAuth2 Relying Party defaults
# These are intentionally empty so that the SAML path works out of the box.
# Set all three variables to enable the oidc-sp authentication source.
: "${SIMPLESAMLPHP_OIDC_ISSUER:=}"
...
export SIMPLESAMLPHP_BASE_URL_PATH SIMPLESAMLPHP_SP_ENTITY_ID \
       ...
       SIMPLESAMLPHP_OIDC_ISSUER SIMPLESAMLPHP_OIDC_CLIENT_ID SIMPLESAMLPHP_OIDC_CLIENT_SECRET
...
# ---------------------------------------------------------------------------
SSMPHP_AUTH_VARS='${SIMPLESAMLPHP_SP_ENTITY_ID} ${SIMPLESAMLPHP_SP_PRIVATEKEY} ${SIMPLESAMLPHP_SP_CERTIFICATE} ${SIMPLESAMLPHP_SP_WANT_ASSERTIONS_SIGNED} ${SIMPLESAMLPHP_SP_WANT_MESSAGE_SIGNED} ${SIMPLESAMLPHP_OIDC_ISSUER} ${SIMPLESAMLPHP_OIDC_CLIENT_ID} ${SIMPLESAMLPHP_OIDC_CLIENT_SECRET}'
